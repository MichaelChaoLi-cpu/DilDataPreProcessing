"""
Scan all dataset folders under RAW_DATA_DIR, extract hl.sav column names and
labels, and write per-dataset hl.yaml files to DATA_DIR/HL/raw/{dataset}/hl.yaml.
Folders without hl.sav are recorded in a log file.
...
OUTPUT_ROOT = DATA_DIR / "HL" / "raw"
LOG_FILE = DATA_DIR / "HL" / "raw" / "extract_hl_columns.log"
...
def find_hl_sav(folder: Path) -> Path | None:
    exact = folder / "hl.sav"
    if exact.exists():
        return exact
    # Some datasets use country-prefixed names like AFhl.sav, BHhl.sav
    matches = [
...
def extract_dataset(folder: Path) -> None:
    sav_path = find_hl_sav(folder)
    if sav_path is None:
        logging.warning("No hl.sav: %s", folder.name)
        return
    if sav_path.name != "hl.sav":
