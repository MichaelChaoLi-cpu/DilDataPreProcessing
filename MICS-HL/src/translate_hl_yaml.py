"""
Process raw hl.yaml files and produce:
  - data/HL/translate/{dataset}/hl.yaml           — labels only (no embedding)
  - data/HL/translate_embedding/{dataset}/hl.csv  — column_in_raw_sav + emb_0..emb_3071

Log file: logs/translate_hl.log  (append, one summary line per dataset)

Skip logic: a dataset is skipped if BOTH output files already exist.

Input:  MICS-HL/data/HL/raw/{dataset}/hl.yaml
Output: MICS-HL/data/HL/translate/{dataset}/hl.yaml
        MICS-HL/data/HL/translate_embedding/{dataset}/hl.csv
"""
...
RAW_DIR       = _MICS_HL_DIR / "data" / "HL" / "raw"
TRANSLATE_DIR = _MICS_HL_DIR / "data" / "HL" / "translate"
EMBED_DIR     = _MICS_HL_DIR / "data" / "HL" / "translate_embedding"
LOG_FILE      = _MICS_HL_DIR / "logs" / "translate_hl.log"
...
def _translate_all(labels: list[str], source_language: str) -> tuple[list[str], int, int, int, int]:
    ...
    for start in range(0, len(labels), _TRANSLATE_BATCH_SIZE):
        ...
        try:
            ...
            # API blocked or empty response — keep original labels for this batch.
            logger.warning(
...
def main() -> None:
    ...
    raw_yaml_paths = sorted(RAW_DIR.rglob("hl.yaml"))
    ...
    for raw_yaml_path in raw_yaml_paths:
        ...
        out_yaml_path = TRANSLATE_DIR / dataset_name / "hl.yaml"
        out_csv_path  = EMBED_DIR / dataset_name / "hl.csv"
