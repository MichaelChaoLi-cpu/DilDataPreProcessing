...
Tables created (drop + recreate if they exist):
  final_CH_MICS  -- merged child-level data from ch_merged.parquet
  ind_que_CH_MICS  -- index of canonical variables with original SAV column
...
def upload_final(conn) -> None:
    ...
    with conn.cursor() as cur:
        _drop_and_create(cur, "final_CH_MICS", df)
        conn.commit()
        _copy_upload(conn, cur, "final_CH_MICS", df)
...
def upload_index(conn) -> None:
    ...
    with conn.cursor() as cur:
        _drop_and_create(cur, "ind_que_CH_MICS", df)
        conn.commit()
        _copy_upload(conn, cur, "ind_que_CH_MICS", df)
