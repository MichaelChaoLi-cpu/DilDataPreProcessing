"""
Upload CH data to PostgreSQL (localhost:5432, database: mda).

Tables created (drop + recreate if they exist):
  final_CH_MICS2MICS6  -- merged child-level data from ch_merged.parquet
  ind_que_CH_MICSMICS  -- index of canonical variables with original SAV column
                          names and English labels, one row per dataset × variable

Usage:
  python upload_ch_to_postgres.py
"""
...
LOG_FILE       = _MICS_CH_DIR / "logs" / "upload_ch.log"
...
def upload_final(conn) -> None:
    ...
    with conn.cursor() as cur:
        _drop_and_create(cur, "final_CH_MICS2MICS6", df)
        conn.commit()
        _copy_upload(conn, cur, "final_CH_MICS2MICS6", df)
...
def upload_index(conn) -> None:
    ...
    with conn.cursor() as cur:
        _drop_and_create(cur, "ind_que_CH_MICSMICS", df)
        conn.commit()
        _copy_upload(conn, cur, "ind_que_CH_MICSMICS", df)
