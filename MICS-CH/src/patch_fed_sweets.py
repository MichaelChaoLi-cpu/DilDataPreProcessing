"""
P48 — `CP_fed_sweets_yesterday` (CH): child ate sugary/sweet FOODS (chocolate, sweets,
candies, pastries, cakes) yesterday, 1/0.

The "sweet foods" item is an unhealthy-foods add-on asked in only a minority of MICS
surveys, so coverage is low. Its raw letter varies (BD8O / BD8P / BD8Q; MICS4 BF16M /
BF19N; Ghana DD1S) and the same letters mean other things elsewhere (BD8O is "insects"
or "nuts" in some datasets), so this is rebuilt FRESH from raw by LABEL, not by letter.

Per dataset, select the column whose label is a sugary/sweet-FOOD item (sugary food /
sweet food / aliments sucrés / chocolate+candy/pastry/cake) and harmonize it. Excluded:
sugary DRINKS (soda/juice/eau sucrée), diarrhoea sugar-salt solutions, sweet potato.

Value: 1=Yes -> 1, 2=No -> 0; sentinels (7/8/9) -> NULL. Guard: SAV row count == parquet
AND household_number == SAV HH id >= 99.9%.

Usage:
    .venv/bin/python MICS-CH/src/patch_fed_sweets.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_sweets.py --verify
"""
...
CP = "CP_fed_sweets_yesterday"
BASE = "dd_sweets"
GUARD_KEYS = ["HH2", "hh2", "HI2", "hi2", "WIHHNO", "wihhno", "hh1", "HH1"]

# sugary/sweet FOODS group (chocolate/candy/pastry/cake). Letter varies (BD8O/P/Q, MICS4).
_YOG = re.compile(r"(sugary food|sweet food|sugar food|aliments? sucre|alimento.*(azucar|dulce)|"
                  r"aliment.*sucre|comida.*dulce|"
                  r"(chocolate|chocolat).*(candy|candies|sweet|pastr|cake|bonbon|caramelo|gateau|"
                  r"patisserie|biscuit|galleta|torta|bolo|miel|honey|pan dulce)|"
                  r"chocolates, sweets|sweets, candies|pastries, cakes)")
_COUNT = re.compile(r"(times|nombre|fois|number|veces|vezes|cuantas|quantas|numero|no\. of|how many)")
# exclude sugary DRINKS, diarrhoea sugar-salt solutions, and sweet potato
_EXCL = re.compile(r"(drink|boisson|bebida|\beau\b|\bagua\b|liquid|refresco|\bjus\b|jugo|"
                   r"\bthe\b|\btea\b|coffee|cafe|syrup|sirop|soda|fanta|sobo|frooty|malt|fizzy|"
                   r"aguas|bebio|bebeu|bu du|tomo|"
                   r"solution|rehydrat|\bsel\b|\bsalt\b|salada|salee|\bors\b|\bsro\b|diarrh|"
                   r"batata doce|patate douce|sweet potato|camote|boniato|dulces que|zanahoria)")
...
def _select_cols(meta):
    """Names of every sugary-FOOD yes/no column (excluding counts / drinks / solutions)."""
    out = []
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p48").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p48"))
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD8O/BD8P/BD8Q',
               'Child ate sugary/sweet foods (chocolate, candies, pastries) yesterday',
               'derived', measure_type, 'Child ate sugary/sweet foods yesterday'
...
def main():
    ...
    print(f"P48 CP_fed_sweets_yesterday — {'VERIFY' if verify else 'APPLY'}")
