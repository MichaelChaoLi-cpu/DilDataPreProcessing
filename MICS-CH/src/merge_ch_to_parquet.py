"""
Merge all CH SAV files into a single parquet using alignment_v2.yaml.

Inputs:
  RAW_DATA_DIR/{dataset}/ch.sav        (from config.py / .env)
  data/CH/alignment_v2.yaml
  data/CH/questionnaire_dedup_v2/{dataset}/dup.yaml

Output:
  data/CH/processed_data/ch_merged.parquet

One row per child; dataset_name is the first column.
"""
...
ALIGNMENT_FILE = _MICS_CH_DIR / "data" / "CH" / "alignment_v2.yaml"
DEDUP_DIR      = _MICS_CH_DIR / "data" / "CH" / "questionnaire_dedup_v2"
OUTPUT_DIR     = DATA_DIR / "CH" / "processed_data"
LOG_FILE       = _MICS_CH_DIR / "logs" / "merge_ch.log"
...
def _find_ch_sav(folder: Path) -> Path | None:
    exact = folder / "ch.sav"
    if exact.exists():
        return exact
...
def _process_dataset(
    ...
    sav_path = _find_ch_sav(folder)
    if sav_path is None:
        logger.warning("No ch.sav: %s", dataset_name)
        return None
...
def main() -> None:
    ...
    out_path = OUTPUT_DIR / "ch_merged.parquet"
