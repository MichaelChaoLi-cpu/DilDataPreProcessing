...
Usage:
    .venv/bin/python MICS-CH/src/patch_fed_plain_water.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_plain_water.py --verify
"""
...
CP = "CP_fed_plain_water_yesterday"
BASE = "infant_fed_plain_water_yesterday"
GUARD_KEYS = ["HH2", "hh2", "HI2", "hi2", "WIHHNO", "wihhno", "hh1", "HH1"]

# the infant-feeding plain-water item lives in this small, stable set of column NAMES
_WATER_NAMES = {"BD7A", "BF3", "BF3B"}
# label must mention (plain) water
_WATER = re.compile(r"(water|eau|agua)")
_COUNT = re.compile(r"(times|nombre|fois|number|veces|vezes|cuantas|quantas|numero|no\. of|how many)")
# reject the false friends that also carry a "water" label
_EXCL = re.compile(r"(diarrh|during diarr|pendant.*diarrh|with feeding|water alone|avec.*aliment|"
                   r"fetch|person fetching|corvee|source|supply|provient|treat|purif|boil|"
                   r"chlor|filter|wash|laver|\bhand\b|\bmain\b|toilet|latrine|sanit|"
                   r"sugar|sucre|azucar|salt|\bsel\b|honey|miel|glucose|glucos|rice|\briz\b|arroz|"
                   r"juice|\bjus\b|jugo|milk|lait|leche|formula|breast|rehydr|\bors\b|\bsro\b|"
                   r"solution|remede|speak|sentence|correctly|doce|doux|sweet|aromatis|flavour|flavor|sabor|canja|caldo|\bsopa\b|soup|potage|\btea\b|\bthe\b|\bcha\b|gripe|anise|anis|tisane|infusion)")
...
def _select_cols(meta):
    """The plain-water yes/no column(s): NAME in the stable set AND label mentions water,
    excluding diarrhoea / water-source / treatment / sugar-or-rice-water false friends."""
    out = []
...
def _from_raw(ds, parquet_ds):
    ...
    if not ycols:
        return None, "no plain-water column"
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p58").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p58"))
    ...
    for ds, note in sorted(skip):
        if note not in ("no plain-water column", "no dir", "no SAV") and "row mismatch" not in note:
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD7A/BF3/BF3B (plain water)',
               'Child drank plain water yesterday',
               'derived', measure_type, 'Child drank plain water yesterday'
...
def main():
    ...
    print(f"P58 CP_fed_plain_water_yesterday — {'VERIFY' if verify else 'APPLY'}")
