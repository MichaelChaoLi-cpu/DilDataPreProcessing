"""
P43 — `CP_fed_eggs_yesterday` (CH): child ate eggs yesterday, 1/0.

Source: **BD8K** — "Child ate eggs yesterday" (the MICS5/6 24-hour eggs food group). Rebuilt
FRESH from raw rather than harmonizing `dd_eggs` (101 datasets), because BD8K is legumes (not
eggs) for the letter-shifted Pakistan-KP MICS5 (its eggs are BD8I), and Azerbaijan MICS6-2023
is multi-source (BD8F1 + BD8K). BD8K is present in 115 raw SAVs.

Guard, per dataset: BD8K present AND its variable label is an eggs item (egg/oeuf/huevo/ovo);
SAV row count == parquet; household_number == SAV HH id >= 99.9%. Shifted-letter datasets read
their real eggs column (Pakistan-KP BD8I, Madagascar-South BF15JX). For single-BD8K-source
datasets whose raw re-read fails only the household guard (id-recoded), the merged dd_eggs
value is kept. Value: 1->1, 2->0, 7/8/9->NULL.

Usage:
    .venv/bin/python MICS-CH/src/patch_fed_eggs.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_eggs.py --verify
"""
...
SPECIAL = {
    "Pakistan_(Khyber_Pakhtunkhwa)_MICS5_Datasets": "BD8I",
    "Madagascar (South)_ MICS4_Datasets": "BF15JX",
}

# eggs — accept BD8K only if its label is an eggs item
_CHEESE = re.compile(r"(\begg\b|eggs|oeufs?|œufs?|huevos?|\bovos?\b)")
...
def _single_bd8k_datasets():
    """Datasets whose dd_dairy was mapped ONLY from BD8N — the merged value is
    trustworthy BD8K and can be kept when the raw re-read guard fails (id-recoded)."""
    ...
    cur.execute("""SELECT dataset_name FROM "ind_que_CH_MICS"
                   ...
                   HAVING array_agg(DISTINCT column_in_raw_sav) = ARRAY['BD8K']""")
...
def _from_raw(ds, parquet_ds):
    ...
    if not _CHEESE.search(f) or _COUNT.search(f):
        return None, f"{want} not eggs: '{lbl[:40]}'"
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p43").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p43"))
    single = _single_bd8k_datasets()
    ...
    for ds in df.dataset_name.unique():
        ...
        if ser is None:
            if ds in single:
                ...
                if base.notna().any():
                    cp[m] = base.values
                    kept.append((ds, f"kept dd (single-BD8K) n={int(base.notna().sum())}; raw:{note}"))
                    continue
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD8K',
               'Child ate eggs yesterday',
               'derived', measure_type, 'Child ate eggs yesterday'
...
def main():
    ...
    print(f"P43 CP_fed_eggs_yesterday — {'VERIFY' if verify else 'APPLY'}")
