...
Usage:
    .venv/bin/python MICS-CH/src/patch_fed_juice.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_juice.py --verify
"""
...
CP = "CP_fed_juice_yesterday"
BASE = "infant_fed_juice_yesterday"
GUARD_KEYS = ["HH2", "hh2", "HI2", "hi2", "WIHHNO", "wihhno", "hh1", "HH1"]

# the infant-feeding juice item lives in this small, stable set of column NAMES
_WATER_NAMES = {"BD7B", "BF8", "BF3C"}
# label must mention juice
_WATER = re.compile(r"(juice|jus|jugo|sumo|suco|zumo)")
...
_EXCL = re.compile(r"(diarrh|during diarr|pendant.*diarrh|with feeding|avec.*aliment|"
                   r"rehydr|\bors\b|\bsro\b|solution|remede|"
                   r"milk|lait|leche|formula|breast)")
...
def _select_cols(meta):
    """The juice yes/no column(s): NAME in the stable set AND label mentions juice,
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p59").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p59"))
    ...
    for ds, note in sorted(skip):
        if note not in ("no juice column", "no dir", "no SAV") and "row mismatch" not in note:
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD7B/BF8/BF3C (juice)',
               'Child drank juice or juice drinks yesterday',
               'derived', measure_type, 'Child drank juice or juice drinks yesterday'
...
def main():
    ...
    print(f"P59 CP_fed_juice_yesterday — {'VERIFY' if verify else 'APPLY'}")
