"""
P45 — `CP_fed_dark_green_leafy_vegetables_yesterday` (CH): child ate dark green leafy
vegetables (spinach, broccoli, Swiss chard, etc.) yesterday, 1/0.

Source: **BD8F** — "Child ate (dark) green leafy vegetables yesterday" (the MICS5/6 24-hour
dark-green-leafy-vegetable food group). Rebuilt FRESH from raw. Note the food-group letters
differ between questionnaire versions: in the STANDARD MICS6 BD8F = green leafy vegetables,
but the letter-shifted Pakistan-KP MICS5 has NO green-leafy item and its BD8F is vitamin-A
FRUIT (mangoes/papaya) — so it is excluded by the label guard. BD8F is present in most SAVs.

Guard, per dataset: BD8F present AND its variable label is a green-leafy item (green leafy/
dark green/spinach/broccoli/feuilles vertes…); SAV row count == parquet; household_number
== SAV HH id >= 99.9%. Madagascar-South MICS4 reads its column BF15EX. For single-BD8F-source
datasets whose raw re-read fails only the household guard, the merged value is kept.
Value: 1->1, 2->0, 7/8/9->NULL.

Usage:
    .venv/bin/python MICS-CH/src/patch_fed_green_leafy.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_green_leafy.py --verify
"""
...
CP = "CP_fed_dark_green_leafy_vegetables_yesterday"
BASE = "dd_green_leafy_veg"
WANT = "BD8F"
...
SPECIAL = {"Madagascar (South)_ MICS4_Datasets": "BF15EX"}

# dark green leafy vegetables — accept BD8F only if its label is a green-leafy item
_CHEESE = re.compile(r"(green leafy|dark green|leafy veg|leafy green|leaves|spinach|broccoli|"
                     r"swiss chard|\bchard\b|amaranth|\bkale\b|collard|"
                     r"feuille|legumes? a feuille|\bhoja|\bfolha|de cor verde|"
                     r"verdura|hortalizas? de hoja|epinard|espinaca|espinafre|"
                     r"couve|repolho|brede|morogo|sukuma)")
...
def _single_bd8f_datasets():
    """Datasets whose dd_dairy was mapped ONLY from BD8N — the merged value is
    trustworthy BD8F and can be kept when the raw re-read guard fails (id-recoded)."""
    ...
    cur.execute("""SELECT dataset_name FROM "ind_que_CH_MICS"
                   ...
                   HAVING array_agg(DISTINCT column_in_raw_sav) = ARRAY['BD8F']""")
...
def _from_raw(ds, parquet_ds):
    ...
    if not _CHEESE.search(f) or _COUNT.search(f):
        return None, f"{want} not green-leafy: '{lbl[:40]}'"
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p45").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p45"))
    single = _single_bd8f_datasets()
    ...
    for ds in df.dataset_name.unique():
        ...
        if ser is None:
            if ds in single:
                ...
                if base.notna().any():
                    cp[m] = base.values
                    kept.append((ds, f"kept dd (single-BD8F) n={int(base.notna().sum())}; raw:{note}"))
                    continue
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD8F',
               'Child ate dark green leafy vegetables yesterday',
               'derived', measure_type, 'Child ate dark green leafy vegetables yesterday'
...
def main():
    ...
    print(f"P45 CP_fed_dark_green_leafy_vegetables_yesterday — {'VERIFY' if verify else 'APPLY'}")
