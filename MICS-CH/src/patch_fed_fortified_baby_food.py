"""
P35 — `CP_fed_grain_based_fortified_baby_food_yesterday` (CH): child ate (grain-based)
commercially fortified baby food yesterday, 1/0.

Source: **BD8B** (a.k.a. BD8B1) — "Child ate fortified baby food (cerelac, gerber, hero,
nestum, etc.) yesterday", the MICS5/6 24-hour item for commercial grain-based fortified
infant cereal. Verified: all 115 datasets carrying BD8B label it unambiguously as
fortified baby food across EN/FR/ES/PT (cerelac/nestum/gerber/"nourriture fortifiée pour
bébé"/"alimento fortificado para bebé"/"comida de bebê fortificada"); no mis-labels. A
broad label sweep found NO dataset with a fortified-baby-food question outside BD8B —
...
Translation-gap recovery: BD8B is mapped for 107 datasets but present in 8 more whose
raw SAV had it unmapped. Recovered by guarded positional backfill, value classified from
that column's own SAV labels. 107 -> up to 115 datasets.

Usage:
    .venv/bin/python MICS-CH/src/patch_fed_fortified_baby_food.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_fortified_baby_food.py --verify
"""
...
BASE = "infant_fed_fortified_baby_food"
CP = "CP_fed_grain_based_fortified_baby_food_yesterday"
GUARD_KEYS = ["HH2", "hh2", "HI2", "hi2", "WIHHNO", "wihhno", "hh1", "HH1"]

# 8 datasets carrying BD8B in the raw SAV but with it unmapped.
_RECOVER_DATASETS = [
    "Cameroon_MICS5_Datasets", "Central African Republic MICS6 Datasets",
    "Cuba_MICS5_Datasets", "Dominican Republic MICS6 Datasets", "Guyana_MICS5_Datasets",
    "Kosovo under UNSC res. 1244 (Roma, Ashkali, and Egyptian Communities)_MICS5_Datasets",
    "Kosovo under UNSC res. 1244_MICS5_Datasets", "Lesotho_MICS6_datasets",
]
RECOVER = {ds: "BD8B" for ds in _RECOVER_DATASETS}
...
def _recover(ds, col, parquet_ds):
    ...
    c = _find(df.columns, col) or _find(df.columns, "BD8B1")   # BD8B, else the BD8B1 variant
    if c is None:
        return None, f"col {col}/BD8B1 absent"
...
def apply(verify):
    ...
    if verify:
        ...
        return []
    if not PARQUET.with_suffix(".parquet.bak_p35").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p35"))
    avail = set(df.dataset_name.unique())
    nfc = {unicodedata.normalize("NFC", a): a for a in avail}
    cp = _harmonize(df[BASE])                       # 107 mapped datasets
    applied, skipped = [], []
...
def sync_db(applied, verify):
    ...
    for ds in applied:
        ...
        cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
            ...
            (BASE, ds, RECOVER.get(unicodedata.normalize("NFC", ds), RECOVER.get(ds, "BD8B")),
             "Child ate fortified baby food yesterday (recovered P35)", "explicit",
             "infant_feeding", "Child ate grain-based fortified baby food yesterday"))
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        column_label_in_english,source_kind,measure_type,canonical_text)
...
def main():
    ...
    print(f"P35 CP_fed_grain_based_fortified_baby_food_yesterday — {'VERIFY' if verify else 'APPLY'}")
