"""
P40 — `CP_fed_organ_meat_yesterday` (CH): child ate liver, kidney, heart or other organ
meat yesterday, 1/0.

Source: **BD8I** — "Child ate liver, kidney, heart or other organ meat yesterday" (the
MICS5/6 24-hour organ-meat food group). Rebuilt FRESH from raw rather than harmonizing
`dd_organ_meat` (100 datasets), because one dataset's BD8I is mislabelled/shifted — Pakistan-
KP MICS5 BD8I is actually "Child ate eggs" (its food-group letters are shifted; it has no
separate organ-meat item) — so a blind harmonize would inject eggs. BD8I is present in 115
raw SAVs.

Guard, per dataset: BD8I present AND its variable label is an organ-meat item
(liver/kidney/heart/foie/rognon/abats/vísceras…); SAV row count == parquet; household_number
== SAV HH id >= 99.9%. Madagascar-South MICS4 reads its shifted column BF15HX. For single-
BD8I-source datasets whose raw re-read fails only the household guard (id-recoded), the merged
dd_organ_meat value is kept (trusted). Value: 1=Yes -> 1, 2=No -> 0, sentinels (7/8/9) -> NULL.

Usage:
    .venv/bin/python MICS-CH/src/patch_fed_organ_meat.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_organ_meat.py --verify
"""
...
CP = "CP_fed_organ_meat_yesterday"
BASE = "dd_organ_meat"
WANT = "BD8I"
...
SPECIAL = {
    "Madagascar (South)_ MICS4_Datasets": "BF15HX",
}

# organ meat — accept BD8I only if its label is an organ-meat item
_CHEESE = re.compile(r"(liver|kidney|heart|organ meat|\borgans?\b|foie|rognon|reins?\b|"
                     r"coeur|c\?ur|abats|abat\b|viscera|visceras|entrailles|gesier|"
                     r"figado|\brim\b|coracao|higado|rinon|corazon|organos|brain|tongue|"
                     r"visceres)")
...
def _single_bd8i_datasets():
    """Datasets whose dd_dairy was mapped ONLY from BD8N — the merged value is
    trustworthy BD8I and can be kept when the raw re-read guard fails (id-recoded)."""
    ...
    cur.execute("""SELECT dataset_name FROM "ind_que_CH_MICS"
                   ...
                   HAVING array_agg(DISTINCT column_in_raw_sav) = ARRAY['BD8I']""")
...
def _from_raw(ds, parquet_ds):
    ...
    if not _CHEESE.search(f) or _COUNT.search(f):
        return None, f"{want} not organ: '{lbl[:40]}'"
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p40").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p40"))
    single = _single_bd8i_datasets()
    ...
    for ds in df.dataset_name.unique():
        ...
        if ser is None:
            if ds in single:
                ...
                if base.notna().any():
                    cp[m] = base.values
                    kept.append((ds, f"kept dd (single-BD8I) n={int(base.notna().sum())}; raw:{note}"))
                    continue
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD8I',
               'Child ate liver, kidney, heart or other organ meat yesterday',
               'derived', measure_type, 'Child ate organ meat (liver/kidney/heart) yesterday'
...
def main():
    ...
    print(f"P40 CP_fed_organ_meat_yesterday — {'VERIFY' if verify else 'APPLY'}")
