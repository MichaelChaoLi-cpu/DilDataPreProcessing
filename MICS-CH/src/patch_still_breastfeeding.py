"""
P30 — `CP_still_breastfeeding` (CH): harmonized "is the child still being breastfed?" 1/0.
Coding uniform 1=Yes/2=No (multilingual), sentinels -> NULL. 240 datasets already
mapped; 2 more (DR Congo 2001 BF2, Kyrgyzstan bf2) have it unmapped in the raw SAV —
recovered by guarded positional backfill. Near ceiling: the other 9 missing datasets
(old MICS2/2000 reduced questionnaires) never asked it.
...
Usage:
    .venv/bin/python MICS-CH/src/patch_still_breastfeeding.py            # apply
    .venv/bin/python MICS-CH/src/patch_still_breastfeeding.py --verify
"""
...
BASE = "still_breastfeeding"
CP = "CP_still_breastfeeding"
GUARD_KEYS = ["HH2", "hh2", "HI2", "hi2", "WIHHNO", "wihhno", "hh1", "HH1"]

# 2 datasets with an unmapped still-breastfeeding column (raw SAV col).
RECOVER = {
    'Congo, Democratic Republic of 2001 MICS_Datasets': 'BF2',
    'Kyrgyzstan MICS 2005-06 SPSS Datasets': 'bf2',
}
...
def apply(verify):
    ...
    if verify:
        ...
        return []
    if not PARQUET.with_suffix(".parquet.bak_p30").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p30"))
...
def sync_db(applied, verify):
    ...
    for ds in applied:
        ...
        cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
            ...
            (BASE, ds, RECOVER.get(unicodedata.normalize("NFC", ds), RECOVER.get(ds, "BF2")),
             "Still breastfeeding (recovered P30)", "explicit", "breastfeeding", "Child still breastfeeding"))
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        column_label_in_english,source_kind,measure_type,canonical_text)
...
def main():
    ...
    print(f"P30 CP_still_breastfeeding — {'VERIFY' if verify else 'APPLY'}")
