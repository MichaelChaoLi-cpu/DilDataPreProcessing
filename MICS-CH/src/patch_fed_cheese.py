"""
P39 — `CP_fed_cheese_other_dairy_yesterday` (CH): child ate cheese or other food made
from milk yesterday, 1/0.

Source: **BD8N** — "Child ate cheese or other food made from milk yesterday" (the MICS5/6
24-hour cheese/dairy-food group; distinct from yogurt = BD8A/BD7F [P38] and milk drinking
[P31]). Rebuilt FRESH from raw rather than harmonizing `dd_dairy` (54 datasets), which is
contaminated: for several multi-source datasets the merge took the wrong raw column
(Cameroon/CAR MICS5/6 == BD8A yogurt; Georgia == BD7P/BD7Q1). BD8N is present in 114 raw
SAVs — a large recovery.

Guard, per dataset: BD8N present AND its variable label is a cheese/dairy-food item
(cheese/fromage/queso/queijo or "food made from milk"); SAV row count == parquet;
household_number == SAV HH id >= 99.9%. For single-BD8N-source datasets whose raw re-read
fails only the household guard (id-recoded), the merged dd_dairy value is kept (trusted).
Value: 1=Yes -> 1, 2=No -> 0, sentinels (7/8/9) -> NULL.

Usage:
    .venv/bin/python MICS-CH/src/patch_fed_cheese.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_cheese.py --verify
"""
...
CP = "CP_fed_cheese_other_dairy_yesterday"
BASE = "dd_dairy"
WANT = "BD8N"
...
SPECIAL = {
    "Pakistan_(Khyber_Pakhtunkhwa)_MICS5_Datasets": "BD8L",
    "Madagascar (South)_ MICS4_Datasets": "BF15MX",
}

# cheese / "other food made from milk" — accept BD8N only if its label is dairy-food
_CHEESE = re.compile(r"(cheese|fromage|queso|queijo|\bkase\b|brynza|\bkurt\b|cottage|"
                     r"caillebotte|requeson|ricotta|"
                     r"food made from milk|aliment.*lait|aliment.*fait avec.*lait|"
                     r"produit laitier|alimentos.*leche|comida.*leche|comida.*leite|"
                     r"alimentos.*leite|hechos con leche|hechos a base de leche|"
                     r"a base de lait|a base de leite|feita de leite)")
...
def _single_bd8n_datasets():
    """Datasets whose dd_dairy was mapped ONLY from BD8N — the merged value is
    trustworthy BD8N and can be kept when the raw re-read guard fails (id-recoded)."""
    ...
    cur.execute("""SELECT dataset_name FROM "ind_que_CH_MICS"
                   ...
                   HAVING array_agg(DISTINCT column_in_raw_sav) = ARRAY['BD8N']""")
...
def _from_raw(ds, parquet_ds):
    ...
    if not _CHEESE.search(f) or _COUNT.search(f):
        return None, f"{WANT} not cheese: '{lbl[:40]}'"
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p39").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p39"))
    single = _single_bd8n_datasets()
    ...
    for ds in df.dataset_name.unique():
        ...
        if ser is None:
            if ds in single:
                ...
                if base.notna().any():
                    cp[m] = base.values
                    kept.append((ds, f"kept dd (single-BD8N) n={int(base.notna().sum())}; raw:{note}"))
                    continue
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD8N',
               'Child ate cheese or other food made from milk yesterday',
               'derived', measure_type, 'Child ate cheese or other dairy food yesterday'
...
def main():
    ...
    print(f"P39 CP_fed_cheese_other_dairy_yesterday — {'VERIFY' if verify else 'APPLY'}")
