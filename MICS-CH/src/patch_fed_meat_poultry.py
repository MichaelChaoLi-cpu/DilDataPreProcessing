"""
P41 — `CP_fed_meat_poultry_yesterday` (CH): child ate meat/poultry (beef, pork, lamb, goat,
chicken, duck) yesterday, 1/0.

Source: **BD8J** — "Child ate meat, such as beef, pork, lamb, goat, chicken, duck yesterday"
(the MICS5/6 24-hour flesh-meat/poultry food group). Rebuilt FRESH from raw rather than
harmonizing `dd_meat_poultry` (102 datasets), because it is mis-sourced for a few: Pakistan-
KP MICS5 BD8J is actually "fish" (letters shifted; its meat is BD8H); Vietnam MICS4 came from
BF9 "meat SOUP/broth" (a liquid, not meat-eating). BD8J is present in 115 raw SAVs.

Guard, per dataset: BD8J present AND its variable label is a meat item (meat/beef/pork/lamb/
goat/chicken/duck/viande/carne…); SAV row count == parquet; household_number == SAV HH id
>= 99.9%. Shifted-letter datasets read their real meat column (Pakistan-KP BD8H, Madagascar-
South BF15IX). For single-BD8J-source datasets whose raw re-read fails only the household
guard (id-recoded), the merged dd_meat_poultry value is kept. Value: 1->1, 2->0, 7/8/9->NULL.

Usage:
    .venv/bin/python MICS-CH/src/patch_fed_meat_poultry.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_meat_poultry.py --verify
"""
...
CP = "CP_fed_meat_poultry_yesterday"
BASE = "dd_meat_poultry"
WANT = "BD8J"
...
SPECIAL = {
    "Pakistan_(Khyber_Pakhtunkhwa)_MICS5_Datasets": "BD8H",
    "Madagascar (South)_ MICS4_Datasets": "BF15IX",
}

# meat/poultry — accept BD8J only if its label is a flesh-meat item (not soup/broth)
_CHEESE = re.compile(r"(\bmeat\b|beef|pork|lamb|goat|chicken|duck|poultry|turkey|veal|"
                     r"viande|boeuf|porc|agneau|chevre|poulet|mouton|volaille|sausage|"
                     r"carne|\bres\b|cerdo|cordero|pollo|vaca|galinha|frango|cabra|cabrito|"
                     r"chivo|ovejo|borrego)")
_NOTMEAT = re.compile(r"(soup|broth|bouillon|water|caldo|soupe)")
...
def _single_bd8j_datasets():
    """Datasets whose dd_dairy was mapped ONLY from BD8N — the merged value is
    trustworthy BD8J and can be kept when the raw re-read guard fails (id-recoded)."""
    ...
    cur.execute("""SELECT dataset_name FROM "ind_que_CH_MICS"
                   ...
                   HAVING array_agg(DISTINCT column_in_raw_sav) = ARRAY['BD8J']""")
...
def _from_raw(ds, parquet_ds):
    ...
    f = _fold(lbl)
    if not _CHEESE.search(f) or _COUNT.search(f) or _NOTMEAT.search(f):
        return None, f"{want} not meat: '{lbl[:40]}'"
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p41").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p41"))
    single = _single_bd8j_datasets()
    ...
    for ds in df.dataset_name.unique():
        ...
        if ser is None:
            if ds in single:
                ...
                if base.notna().any():
                    cp[m] = base.values
                    kept.append((ds, f"kept dd (single-BD8J) n={int(base.notna().sum())}; raw:{note}"))
                    continue
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD8J',
               'Child ate meat, such as beef, pork, lamb, goat, chicken, duck yesterday',
               'derived', measure_type, 'Child ate meat or poultry yesterday'
...
def main():
    ...
    print(f"P41 CP_fed_meat_poultry_yesterday — {'VERIFY' if verify else 'APPLY'}")
