"""
P55 — `CP_times_animal_milk_yesterday` (CH): number of times the child drank animal / tinned /
powdered / fresh milk yesterday (a COUNT, not yes/no). Count companion of P53
CP_fed_animal_milk_yesterday.

Source by LABEL: the "times child drank milk from animals" column — BD7E1 (MICS6), BF7
(MICS4/5). No prior canonical existed; built fresh from raw. Present in ~51 datasets.
...
NULLed if its label says DK/missing/NR/NSP/inconsistent, or if code >= 90; otherwise the
numeric count (1..~30) is kept.

Guard: SAV row count == parquet AND household id matches >= 99.9%.

Usage:
    .venv/bin/python MICS-CH/src/patch_times_animal_milk.py            # apply
    .venv/bin/python MICS-CH/src/patch_times_animal_milk.py --verify
"""
...
CP = "CP_times_animal_milk_yesterday"
BASE = "times_animal_milk_yesterday"
GUARD_KEYS = ["HH2", "hh2", "HI2", "hi2", "WIHHNO", "wihhno", "hh1", "HH1"]

# "times child drank animal / tinned / powdered / fresh milk" — a count column.
...
_FORM = re.compile(r"(milk from animals|animal milk|tinned.*milk|powdered.*milk|fresh.*milk|"
                   r"milk.*(tinned|powder|fresh|animal)|"
                   r"lait.*(en boite|en poudre|frais|animal|de vache|concentre)|"
                   r"leche.*(en polvo|fresca|animal|de vaca|envasada|de lata)|"
                   r"leite.*(em po|fresco|animal|de vaca))")
_EXCL = re.compile(r"(formula|formule|maternise|infantil|lactogen|breast|maternel|materno|amamant|"
                   r"yogurt|yaourt|water|juice|jugo|diarrh|solid|porridge|homemade|recommended|"
                   r"fermented|government)")
...
def _from_raw(ds, parquet_ds):
    ...
    if col is None:
        return None, "no times-animal-milk column"
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p55").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p55"))
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD7E1/BF7 (times animal milk)',
               'Number of times child drank animal milk yesterday',
               'derived', measure_type, 'Number of times child drank animal/other milk yesterday'
...
def main():
    ...
    print(f"P55 CP_times_animal_milk_yesterday — {'VERIFY' if verify else 'APPLY'}")
