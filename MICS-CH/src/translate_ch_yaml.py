"""
Process raw ch.yaml files and produce:
  - data/CH/translate/{dataset}/ch.yaml           — labels only (no embedding)
  - data/CH/translate_embedding/{dataset}/ch.csv  — column_in_raw_sav + emb_0..emb_3071

Log file: logs/translate_ch.log  (append, one summary line per dataset)

Skip logic: a dataset is skipped if BOTH output files already exist.

Input:  MICS-CH/data/CH/raw/{dataset}/ch.yaml
Output: MICS-CH/data/CH/translate/{dataset}/ch.yaml
        MICS-CH/data/CH/translate_embedding/{dataset}/ch.csv
"""
...
_MICS_CH_DIR = Path(__file__).parent.parent
RAW_DIR       = _MICS_CH_DIR / "data" / "CH" / "raw"
TRANSLATE_DIR = _MICS_CH_DIR / "data" / "CH" / "translate"
EMBED_DIR     = _MICS_CH_DIR / "data" / "CH" / "translate_embedding"
LOG_FILE      = _MICS_CH_DIR / "logs" / "translate_ch.log"
...
def main() -> None:
    ...
    raw_yaml_paths = sorted(RAW_DIR.rglob("ch.yaml"))
    ...
    for raw_yaml_path in raw_yaml_paths:
        ...
        out_yaml_path = TRANSLATE_DIR / dataset_name / "ch.yaml"
        out_csv_path  = EMBED_DIR / dataset_name / "ch.csv"
