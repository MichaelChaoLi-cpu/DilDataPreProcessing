...
Usage:
    .venv/bin/python MICS-CH/src/patch_fed_soup.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_soup.py --verify
"""
...
CP = "CP_fed_clear_broth_soup_yesterday"
BASE = "infant_fed_soup_yesterday"
GUARD_KEYS = ["HH2", "hh2", "HI2", "hi2", "WIHHNO", "wihhno", "hh1", "HH1"]

# the infant-feeding broth/soup item lives in this small set of column NAMES
_WATER_NAMES = {"BD7C", "BF9", "BF3G"}
# label must mention broth / soup
_WATER = re.compile(r"(broth|\bsoup\b|clear soup|caldo|\bsopa\b|canja|bouillon|\bsoupe\b|"
                    r"potage|\bcaldos\b|zuppa|brodo)")
...
_EXCL = re.compile(r"(diarrh|during diarr|pendant.*diarrh|para la diarrea|pour la diarrh|"
                   r"tea|gripe|anise|type of|tipo de|quel type|solid or semi|semi-?solid|times|nombre de fois|number of times)")
...
def _select_cols(meta):
    """The feeding broth/soup yes/no column(s): NAME in the set AND label mentions broth/soup,
    excluding diarrhoea-care / tea / times-count false friends."""
    out = []
...
def _from_raw(ds, parquet_ds):
    ...
    if not ycols:
        return None, "no broth/soup column"
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p61").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p61"))
    ...
    for ds, note in sorted(skip):
        if note not in ("no broth/soup column", "no dir", "no SAV") and "row mismatch" not in note:
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD7C/BF9 (clear broth/soup)',
               'Child drank clear broth or clear soup yesterday',
               'derived', measure_type, 'Child drank clear broth or clear soup yesterday'
...
def main():
    ...
    print(f"P61 CP_fed_clear_broth_soup_yesterday — {'VERIFY' if verify else 'APPLY'}")
