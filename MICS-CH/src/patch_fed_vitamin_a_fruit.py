"""
P46 — `CP_fed_vitamin_a_fruits_yesterday` (CH): child ate vitamin-A-rich fruits (ripe
mangoes, papayas, apricots, ripe melon, etc.) yesterday, 1/0.

Source: **BD8G** — "Child ate ripe mangoes, papayas etc. any other vitamin-A-rich fruits
yesterday" (the MICS5/6 24-hour vitamin-A-rich fruit food group; the exact fruits are locally
adapted — mango/papaya in the tropics, apricot/melon/persimmon in temperate regions). Rebuilt
FRESH from raw. dd_vitamin_a_fruit covered only 65 of 115 BD8G datasets. Letter-shift: the
Pakistan-KP MICS5 BD8G is "any other fruits or vegetables" (its vit-A fruit is BD8F).

Guard, per dataset: BD8G present AND its variable label is a vit-A-fruit item; SAV row count ==
parquet; household_number == SAV HH id >= 99.9%. Shifted-letter datasets read their real vit-A
fruit column (Pakistan-KP BD8F, Madagascar-South BF15FX). For single-BD8G-source datasets whose
raw re-read fails only the household guard, the merged value is kept. Value: 1->1,2->0,7/8/9->NULL.

Usage:
    .venv/bin/python MICS-CH/src/patch_fed_vitamin_a_fruit.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_vitamin_a_fruit.py --verify
"""
...
CP = "CP_fed_vitamin_a_fruits_yesterday"
BASE = "dd_vitamin_a_fruit"
WANT = "BD8G"
...
SPECIAL = {
    "Pakistan_(Khyber_Pakhtunkhwa)_MICS5_Datasets": "BD8F",
    "Madagascar (South)_ MICS4_Datasets": "BF15FX",
}

# vitamin-A fruit — accept BD8G only if its label is a vit-A-rich FRUIT item (not the
# shifted Pakistan-KP "other fruits or vegetables")
_CHEESE = re.compile(r"(mango|mangue|mangues|manga|mangas|papaya|papaye|papyer|papaia|pawpaw|mamao|mamon|fruta bomba|lechosa|"
                     r"apricot|abricot|damasco|albaricoque|cantaloupe|melon|melons|"
                     r"persimmon|kaki|caqui|peach|peche|durazno|melocoton|sour cherr|cerise|"
                     r"cajamanga|guava|goyave|goiaba|caju maduro|prune|ciruela|"
                     r"vitamin a|vitamina a|vitamine a|riches en vitamin|rich fruit|"
                     r"mures|maduro|madura|maduros|maduras)")
...
def _single_bd8g_datasets():
    """Datasets whose dd_dairy was mapped ONLY from BD8N — the merged value is
    trustworthy BD8G and can be kept when the raw re-read guard fails (id-recoded)."""
    ...
    cur.execute("""SELECT dataset_name FROM "ind_que_CH_MICS"
                   ...
                   HAVING array_agg(DISTINCT column_in_raw_sav) = ARRAY['BD8G']""")
...
def _from_raw(ds, parquet_ds):
    ...
    if not _CHEESE.search(f) or _COUNT.search(f):
        return None, f"{want} not vitA-fruit: '{lbl[:40]}'"
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p46").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p46"))
    single = _single_bd8g_datasets()
    ...
    for ds in df.dataset_name.unique():
        ...
        if ser is None:
            if ds in single:
                ...
                if base.notna().any():
                    cp[m] = base.values
                    kept.append((ds, f"kept dd (single-BD8G) n={int(base.notna().sum())}; raw:{note}"))
                    continue
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD8G',
               'Child ate ripe mangoes, papayas or other vitamin-A-rich fruits yesterday',
               'derived', measure_type, 'Child ate vitamin-A-rich fruits yesterday'
...
def main():
    ...
    print(f"P46 CP_fed_vitamin_a_fruits_yesterday — {'VERIFY' if verify else 'APPLY'}")
