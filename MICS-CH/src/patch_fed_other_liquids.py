...
Usage:
    .venv/bin/python MICS-CH/src/patch_fed_other_liquids.py            # apply
    .venv/bin/python MICS-CH/src/patch_fed_other_liquids.py --verify
"""
...
CP = "CP_fed_other_liquids_yesterday"
BASE = "infant_fed_other_liquid_yesterday"
GUARD_KEYS = ["HH2", "hh2", "HI2", "hi2", "WIHHNO", "wihhno", "hh1", "HH1"]

# the other-liquid item lives in these column NAMES (reused across rounds; label disambiguates)
_WATER_NAMES = {"BD7X", "BF12", "BD7F", "BF3F", "BF3G"}
# label must mention "other liquid"
_WATER = re.compile(r"(other liquid|others liquid|any other liquid|other fluid|"
                    r"autres? liquide|autre.*liquide|otros? liquido|otro liquido|"
                    r"outros? liquido|outro liquido)")
...
def _select_cols(meta):
    """The other-liquid yes/no column(s): NAME in the set AND label mentions 'other liquid',
...
def _from_raw(ds, parquet_ds):
    ...
    if not ycols:
        return None, "no other-liquid column"
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p62").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p62"))
    ...
    for ds, note in sorted(skip):
        if note not in ("no other-liquid column", "no dir", "no SAV") and "row mismatch" not in note:
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD7X/BF12/BD7F/BF3F/BF3G (other liquid)',
               'Child drank any other liquid yesterday',
               'derived', measure_type, 'Child drank any other liquid yesterday'
...
def main():
    ...
    print(f"P62 CP_fed_other_liquids_yesterday — {'VERIFY' if verify else 'APPLY'}")
