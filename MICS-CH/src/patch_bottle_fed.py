...
Usage:
    .venv/bin/python MICS-CH/src/patch_bottle_fed.py            # apply
    .venv/bin/python MICS-CH/src/patch_bottle_fed.py --verify
"""
...
CP = "CP_bottle_fed_yesterday"
BASE = "infant_fed_from_bottle_with_nipple_yesterday"
GUARD_KEYS = ["HH2", "hh2", "HI2", "hi2", "WIHHNO", "wihhno", "hh1", "HH1"]

# the bottle-feeding item lives in these column NAMES
_WATER_NAMES = {"BD4", "BD4A", "BF18", "BF4", "BF3L"}
# label must mention a feeding bottle (with nipple/teat)
_WATER = re.compile(r"(bottle with|bottle.*nipple|bottle.*teat|feeding bottle|from a bottle|"
                    r"from the bottle|biberon|mamadera|mamadeira|tetero|botella|bibron)")
...
_EXCL = re.compile(r"(diarrh|during diarr|pendant.*diarrh|para la diarrea|pour la diarrh|"
                   r"type of|tipo de|quel type|specify|precise|times|nombre de fois|number of times)")
...
def _select_cols(meta):
    """The bottle-feeding yes/no column(s): NAME in the set AND label mentions a feeding bottle,
    excluding diarrhoea-care / type-of / times-count false friends."""
    out = []
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p63").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p63"))
    ...
    for ds, note in sorted(skip):
        if note not in ("no bottle column", "no dir", "no SAV") and "row mismatch" not in note:
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD4/BF18/BF4 (bottle with nipple)',
               'Child was fed from a bottle with a nipple yesterday',
               'derived', measure_type, 'Child was fed from a bottle with a nipple/teat yesterday'
...
def main():
    ...
    print(f"P63 CP_bottle_fed_yesterday — {'VERIFY' if verify else 'APPLY'}")
