"""
P57 — `CP_times_solid_semisolid_soft_food_yesterday` (CH): number of times the child ate
solid, semi-solid or soft foods yesterday (the IYCF meal-frequency COUNT, not yes/no).

Source by LABEL: the "times child ate solid/semi-solid/soft food" column — BD9 (MICS6),
BF17 (MICS4/5), BD11, BF5 (some MICS4/5). Note BF5 doubles as infant-formula-times in other
rounds, so the column is picked strictly by LABEL. Raw times_solid_food_yesterday was mapped
for only 40 datasets; the column is present in ~175. Meal frequency is a single count per
dataset (no eaten+drunk split), so exactly one column is selected.

Sentinel handling is per-dataset via the value labels (the coding differs by round):
  - MICS6 BD9: 7 = "7 OR MORE", 8 = DK, 9 = NO RESPONSE   -> 8/9 are sentinels;
  - MICS4 BF17: real counts, only 98/99 = DK/missing       -> 8-12 kept as counts.
...
Usage:
    .venv/bin/python MICS-CH/src/patch_times_solid_food.py            # apply
    .venv/bin/python MICS-CH/src/patch_times_solid_food.py --verify
"""
...
CP = "CP_times_solid_semisolid_soft_food_yesterday"
BASE = "times_solid_food_yesterday"
GUARD_KEYS = ["HH2", "hh2", "HI2", "hi2", "WIHHNO", "wihhno", "hh1", "HH1"]

# "times child ate solid/semi-solid/soft food" — a count column. TIMES word + SOLID word.
...
_FORM = re.compile(r"(solid|semi-?solid|semisolid|soft food|solides?|semi-?solides?|"
                   r"solidos|semisolidos|blandos|sol[ií]dos|pastosos|aliment.*solide|"
                   r"comida.*solida|alimentos solidos)")
_EXCL = re.compile(r"(formula|formule|maternise|breast|maternel|amamant|milk|lait|leche|leite|"
                   r"yogurt|yaourt|water|\beau\b|juice|jugo|diarrh|dummy|assurer|ensure|"
                   r"make sure|porridge only|liquid|liquide)")
...
def _from_raw(ds, parquet_ds):
    ...
    if col is None:
        return None, "no times-solid-food column"
...
def apply(verify):
    ...
    if verify:
        ...
        return
    if not PARQUET.with_suffix(".parquet.bak_p57").exists():
        shutil.copy2(PARQUET, PARQUET.with_suffix(".parquet.bak_p57"))
...
def sync_db(verify):
    ...
    cur.execute(f'''INSERT INTO {I} (canonical_varname,dataset_name,column_in_raw_sav,
        ...
        SELECT %s, dataset_name, 'BD9/BF17/BD11/BF5 (times solid food)',
               'Number of times child ate solid/semi-solid/soft food yesterday',
               'derived', measure_type, 'Number of times child ate solid/semi-solid/soft food yesterday'
...
def main():
    ...
    print(f"P57 CP_times_solid_semisolid_soft_food_yesterday — {'VERIFY' if verify else 'APPLY'}")
