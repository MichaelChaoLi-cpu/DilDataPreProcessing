"""
Scan all dataset folders under RAW_DATA_DIR, extract wm.sav column names and
labels, and write per-dataset wm.yaml files to DATA_DIR/WM/raw/{dataset}/wm.yaml.
Folders without wm.sav are recorded in a log file.

Input:  {RAW_DATA_DIR}/{dataset}/wm.sav  (or country-prefixed variant)
...
OUTPUT_ROOT = DATA_DIR / "WM" / "raw"
LOG_FILE = DATA_DIR / "WM" / "raw" / "extract_wm_columns.log"

# Suffixes that identify other questionnaires — exclude them from wm fallback search
_OTHER_Q = ("hh", "hl", "ch", "bh", "mn")


def find_wm_sav(folder: Path) -> Path | None:
    # Exact match (case-insensitive on macOS APFS covers WM.sav as well)
    exact = folder / "wm.sav"
    if exact.exists():
        return exact

    # Country-prefixed / suffixed variants, e.g. BHwm.sav, wmAL.sav, GUWM.SAV
    matches = [
        p for p in folder.iterdir()
        if p.is_file()
        and p.suffix.lower() == ".sav"
        and not any(p.stem.lower().endswith(q) for q in _OTHER_Q)
        and (p.stem.lower().endswith("wm") or p.stem.lower().startswith("wm"))
    ]
...
def extract_dataset(folder: Path) -> None:
    sav_path = find_wm_sav(folder)
    if sav_path is None:
        logging.warning("No wm.sav: %s", folder.name)
        return
    if sav_path.name.lower() != "wm.sav":
