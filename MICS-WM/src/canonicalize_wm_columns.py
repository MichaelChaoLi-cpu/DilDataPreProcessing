"""
Build rule-based canonical WM column metadata from translated labels.

Input:  MICS-WM/data/WM/translate/{dataset}/wm.yaml
Output: MICS-WM/data/WM/canonical/{dataset}/wm.yaml
"""
...
from canonical_wm import canonicalize
...
LOG_FILE = _MICS_WM_DIR / "logs" / "canonicalize_wm.log"
...
def process_dataset(translate_yaml_path: Path, out_yaml_path: Path) -> dict[str, int]:
    ...
    for col in data.get("columns", []):
        ...
        entries = canonicalize(english_label)
...
def main() -> None:
    ...
    translate_paths = sorted(TRANSLATE_DIR.rglob("wm.yaml"))
    if args.dataset:
    ...
    for translate_yaml_path in translate_paths:
        ...
        out_yaml_path = CANONICAL_DIR / dataset_name / "wm.yaml"
