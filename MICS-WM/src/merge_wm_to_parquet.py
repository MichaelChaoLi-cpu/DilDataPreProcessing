"""
Merge all WM SAV files into a single parquet using alignment_v2.yaml.

Inputs:
  RAW_DATA_DIR/{dataset}/wm.sav        (from config.py / .env)
  data/WM/alignment_v2.yaml
  data/WM/questionnaire_dedup_v2/{dataset}/dup.yaml

Output:
  data/WM/processed_data/wm_merged.parquet

One row per woman; dataset_name is the first column.
"""
...
ALIGNMENT_FILE = _MICS_WM_DIR / "data" / "WM" / "alignment_v2.yaml"
DEDUP_DIR      = _MICS_WM_DIR / "data" / "WM" / "questionnaire_dedup_v2"
OUTPUT_DIR     = DATA_DIR / "WM" / "processed_data"
LOG_FILE       = _MICS_WM_DIR / "logs" / "merge_wm.log"
...
def _find_wm_sav(folder: Path) -> Path | None:
    exact = folder / "wm.sav"
    if exact.exists():
        return exact
...
def _process_dataset(
    ...
    sav_path = _find_wm_sav(folder)
    if sav_path is None:
        logger.warning("No wm.sav: %s", dataset_name)
        return None
...
def main() -> None:
    ...
    out_path = OUTPUT_DIR / "wm_merged.parquet"
