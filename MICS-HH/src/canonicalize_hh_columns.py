"""
Build rule-based canonical HH column metadata from translated labels.

Input:
  data/HH/translate/{dataset}/hh.yaml

Output:
  data/HH/canonical/{dataset}/hh.yaml

This layer is intentionally separate from translate/ so the original LLM
translation and embedding outputs remain unchanged.
"""
...
from canonical_hh import canonicalize_label
...
LOG_FILE = _MICS_HH_DIR / "logs" / "canonicalize.log"
...
def process_dataset(translate_yaml_path: Path, out_yaml_path: Path) -> dict[str, int]:
    ...
    for col in data.get("columns", []):
        ...
        n_derived += sum(1 for entry in entry_dicts if entry.get("source_kind") == "derived")
        n_review += sum(1 for entry in entry_dicts if entry.get("needs_review"))
        n_compound += sum(1 for entry in entry_dicts if entry.get("is_compound"))
...
def main() -> None:
    ...
    translate_paths = sorted(TRANSLATE_DIR.rglob("hh.yaml"))
    if args.dataset:
    ...
    for translate_yaml_path in translate_paths:
        ...
        out_yaml_path = CANONICAL_DIR / dataset_name / "hh.yaml"
        ...
        try:
            ...
            logger.info(
                "DONE dataset=%s columns=%d entries=%d derived=%d compound=%d needs_review=%d",
                dataset_name,
                stats["columns"],
                stats["entries"],
                stats["derived"],
