...
HISTORY = [
    ("P63", "final_CH_MICS", "CP_bottle_fed_yesterday",
     "The bottle-feeding yes/no item (child fed from a bottle with a nipple/teat; raw BD4 MICS6, BF18 MICS4/5, BF4 MICS2/3) was mapped for 149 datasets though present in ~186; a few MICS6 rounds name it BD4A (Kyrgyz/Samoa/Zimbabwe), and BD4A/BD4B are also used for count/specify sub-items.",
     "Added CP_bottle_fed_yesterday (1/0) rebuilt fresh from raw, NAME-anchored to {BD4,BD4A,BF18,BF4,BF3L} and label-verified on a feeding bottle, excluding diarrhoea-care and count/specify. 167 datasets, 761,466 rows, yes-rate 0.24, no all-yes/all-no dataset."),
    ("P62", "final_CH_MICS", "CP_fed_other_liquids_yesterday",
     "The \u2018any other liquid\u2019 catch-all yes/no feeding item (raw BD7X MICS6, BF12 MICS4/5, BD7F, BF3F/BF3G MICS2/3) was mapped for 229 datasets, but ~11 of those took the value from a diarrhoea-care (CI3H/CA) or count/specify (BD7X1/BF3F1/BF3FA) column, and the name positions are heavily reused across rounds.",
     "Added CP_fed_other_liquids_yesterday (1/0) rebuilt fresh from raw, NAME-anchored to {BD7X,BF12,BD7F,BF3F,BF3G} and label-verified on \u2018other liquid\u2019 (one label per column disambiguates the reused positions), excluding diarrhoea-care and count/specify. 204 datasets, 966,371 rows, yes-rate 0.29, no all-yes/all-no dataset."),
    ("P61", "final_CH_MICS", "CP_fed_clear_broth_soup_yesterday",
     "The clear-broth/clear-soup yes/no feeding item (raw BD7C MICS6, BF9 MICS4/5, BF3G MICS2/3) was mapped for 120 datasets though present in ~155; the BD7C liquid position is reused for other liquids (aniseed tea / gripe water) in a few questionnaires, and \u2018soup\u2019 also appears on the solid-food example list (BF16) and a soup-type follow-up (BF9A).",
     "Added CP_fed_clear_broth_soup_yesterday (1/0) rebuilt fresh from raw, NAME-anchored to {BD7C,BF9,BF3G} and label-verified, excluding diarrhoea-care / tea / solid-food / type-of false friends. 145 datasets, 682,681 rows, yes-rate 0.30, no all-yes/all-no dataset."),
...
CURATED: dict[tuple[str, str], str] = {
    ...
    ("final_CH_MICS", "CP_bottle_fed_yesterday"): "Whether the child was fed anything from a bottle with a nipple/teat yesterday (1=yes, 0=no) \u2014 the IYCF bottle-feeding indicator. Rebuilt fresh from raw, NAME-anchored to {BD4/BD4A MICS6, BF18 MICS4/5, BF4/BF3L MICS2/3} then label-verified on a feeding bottle (bottle with nipple/teat, biberon, mamadera). Diarrhoea-care and count/specify variants excluded; sentinels nulled. 167 datasets. (P63)",
    ("final_CH_MICS", "CP_fed_other_liquids_yesterday"): "Whether the child drank any other liquid yesterday (1=yes, 0=no) \u2014 the catch-all item of the infant liquid checklist. Rebuilt fresh from raw, NAME-anchored to {BD7X MICS6, BF12 MICS4/5, BD7F, BF3F/BF3G MICS2/3} then label-verified on \u2018any other liquid\u2019: these name positions are reused across rounds (BD7F=yogurt-drink, BF3F=animal-milk, BF3G=broth elsewhere), but each column has one label per dataset so the verify disambiguates. Diarrhoea-care and count/specify variants excluded; sentinels nulled. 204 datasets. (P62)",
    ("final_CH_MICS", "CP_fed_clear_broth_soup_yesterday"): "Whether the child drank clear broth or clear soup yesterday (1=yes, 0=no). Rebuilt fresh from raw, NAME-anchored to the broth/soup feeding item (BD7C MICS6 \u2018clear broth/clear soup\u2019; BF9 MICS4/5 \u2018soup/broth\u2019; BF3G MICS2/3) then label-verified \u2014 the BD7C position is reused for other liquids (e.g. aniseed tea) in a few questionnaires, and diarrhoea-care soup (CA/CI) is excluded. Sentinels nulled. 145 datasets. (P61)",
