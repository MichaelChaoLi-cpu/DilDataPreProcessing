...
## P54 — `CP_times_infant_formula_yesterday` (CH)

**Date:** 2026-08-23 · **Module:** CH · **Column:** `CP_times_infant_formula_yesterday`

### Problem
The "number of times the child drank infant formula yesterday" COUNT was mapped for only
**7 datasets** though present in ~142 (raw BD7D1 MICS6; BF5 / BD7EN MICS4/5). Sentinel coding
differs by round: MICS6 BD7D1 codes 7="7 OR MORE", 8=DK, 9=NO RESPONSE; MICS4 BF5 records real
counts 1-12 with only 99=Missing.

### Fix (rebuild fresh from raw, by LABEL; per-dataset sentinels)
Select the times+infant-formula column by label (excluding breast/animal-milk/water/juice/
diarrhoea/solid). A code is NULLed if its value label says DK/missing/NR/NSP/inconsistent or
if code ≥ 90; otherwise the numeric count is kept (1..~30). This nulls MICS6 8/9 (labelled
DK/NR) while keeping MICS4's real 8-12 (unlabelled). Guarded positional backfill.

### Result
**132 datasets**, **77,462 rows** (non-null only for formula-fed children — the question is
skipped otherwise), range 1-22, mean 3.5. Value distribution peaks at 2-3 times/day. Consistency:
98.8% of children with a count were also flagged as having drunk infant formula.

### DB / Parquet: ✅ Done (2026-08-23)
Parquet snapshot `ch_merged.parquet.bak_p54`. `final_CH_MICS` rebuilt via `TRUNCATE` + grouped
...
## P61 — `CP_fed_clear_broth_soup_yesterday` (CH)

**Date:** 2026-08-24 · **Module:** CH · **Column:** `CP_fed_clear_broth_soup_yesterday`

### Problem
The "child drank clear broth / clear soup yesterday" yes/no feeding item was mapped for **120
datasets** though present in ~155 (raw `BD7C` MICS6 "clear broth/clear soup", `BF9` MICS4/5
"soup/broth", `BF3G` MICS2/3). Two hazards: the `BD7C` liquid position is reused for other
liquids ("aniseed tea, gripe water" in Guyana), and the word "soup" also appears on the
solid-food example list (`BF16` "solid or semi-solid such as ... soup") and on a soup-type
follow-up (`BF9A` "what type of soup").

### Fix (rebuild fresh from raw; NAME-anchored + label-verified)
Select a column whose NAME is in `{BD7C, BF9, BF3G}` (the stable broth/soup feeding positions)
AND whose label mentions broth/soup (broth / soup / caldo / sopa / canja / bouillon / soupe /
potage, mojibake-robust fold) AND is a yes/no item, EXCLUDING diarrhoea-care, tea, solid-food
("solid or semi"), and "type of / tipo de" follow-up false friends. 1=Yes→1, 2=No→0; sentinels
→ NULL. Guarded positional backfill.

### Result
**145 datasets**, **682,681 rows**, all 0/1, yes-rate 0.30; no dataset is all-yes or all-no
(mojibake-bug check clean). The ~10 label-matched datasets not sourced failed the row-count /
household-id guard (their base column is retained unchanged).

### DB / Parquet: ✅ Done (2026-08-24)
Parquet snapshot `ch_merged.parquet.bak_p61`. `final_CH_MICS` rebuilt via `TRUNCATE` + grouped
`COPY` (1,684,203 rows / 251 datasets preserved); `ind_que` mirrored. Column type SMALLINT.

### Code
`MICS-CH/src/patch_fed_soup.py` — `_select_cols()` (name-anchored `{BD7C,BF9,BF3G}` +
label-verified), `_classify()`, `_from_raw()` (OR across selected cols), `--verify`.

---

## P62 — `CP_fed_other_liquids_yesterday` (CH)

**Date:** 2026-08-24 · **Module:** CH · **Column:** `CP_fed_other_liquids_yesterday`

### Problem
The "child drank any other liquid yesterday" catch-all yes/no item (raw `BD7X` MICS6, `BF12`
MICS4/5, `BD7F`, `BF3F`/`BF3G` MICS2/3) was mapped for 229 datasets, but ~11 of those took the
value from a diarrhoea-care column (`CI3H`/`CA` series) or a count/specify variant
(`BD7X1`/`BF3F1`/`BF3FA`). The column NAME positions are also heavily reused across rounds:
`BD7F` is the yogurt-drink item (P38/P56) in some questionnaires, `BF3F` is animal milk (P53)
elsewhere, `BF3G` is broth/soup (P61) elsewhere.

### Fix (rebuild fresh from raw; NAME-anchored + label-verified)
Select a column whose NAME is in `{BD7X, BF12, BD7F, BF3F, BF3G}` AND whose label mentions
"other liquid" (any other liquid / autre liquide / otro líquido, mojibake-robust fold) AND is a
yes/no item, EXCLUDING diarrhoea-care and count/"type of"/"specify" false friends. Because each
column carries exactly one label per dataset, the label-verify cleanly assigns each reused name
to whichever patch its label matches — never to two. 1=Yes→1, 2=No→0; sentinels → NULL. Guarded
positional backfill.

### Result
**204 datasets**, **966,371 rows**, all 0/1, yes-rate 0.29; no dataset is all-yes or all-no
(mojibake-bug check clean). The datasets not sourced were either wrongly-mapped in the base
(diarrhoea/count columns, correctly dropped) or failed the row-count / household-id guard; the
base column is retained unchanged.

### DB / Parquet: ✅ Done (2026-08-24)
Parquet snapshot `ch_merged.parquet.bak_p62`. `final_CH_MICS` rebuilt via `TRUNCATE` + grouped
`COPY` (1,684,203 rows / 251 datasets preserved); `ind_que` mirrored. Column type SMALLINT.
